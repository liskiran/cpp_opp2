...
std::unique_ptr<Task> generate_task(Container<std::unique_ptr<Task>>& container) {
  ...
  switch (auto operation = static_cast<TaskNumber>(task_number); operation) {
    case BinaryTask: {
      ...
      switch (auto random_binary_operation = random(1, 4); random_binary_operation) {
        case 1: {
    ...
    case AddOperationTask: {
      return std::make_unique<Add<Task>>(std::move(generate_task(container)), container);
      break;
    }
    ...
    case CounterTask: {
      return std::make_unique<Counter<Task>>(Counter(container));
      break;
    }
    ...
    case ObjectCounterContainerTask: {
      return std::make_unique<ObjectCounterContainer<Task>>(ObjectCounterContainer(container));
      break;
    }
  }
}
